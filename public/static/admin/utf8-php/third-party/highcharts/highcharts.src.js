 ...
 * @param {Object} b The object to cate to the first one
 */
function extend(a, b) {
	var n;
	if (!a) {
...
function getTimeTicks(normalizedInterval, min, max, startOfWeek) {
	...
	if (defined(min)) { // #1300
		...
		// iterate and cate tick positions at appropriate values
		while (time < max) {
...
SVGRenderer.prototype = {
	...
	init: function (container, width, height, forExport) {
		...
		// For browsers other than IE, cate the namespace attribute (#1978)
			...
			each(spans, function (span) {
				if (span !== '' || spans.length === 1) {
					...
					if (span !== ' ') {
						...
						// first span on subsequent line, cate the line height
						if (!spanNo && lineNo) {
	...
	symbol: function (symbol, x, y, width, height, options) {
		...
		// image symbols
			...
			// Ireate the image synchronously, cate attribs async
	...
	label: function (str, x, y, shape, anchorX, anchorY, useHTML, baseline, className) {
		 ...
		 * and cate it before the text in the DOM.
		 */
...
var VMLRendererExtension = { // inherit SVGRenderer
	...
	init: function (container, width, height) {
		...
		// seems to be to cate the xmlns attribute and the behaviour style inline.
	...
	color: function (color, elem, prop, wrapper) {
		...
		// if the color is an rgba color, split it and cate a fill node
	...
	prepVML: function (markup) {
		...
		if (isIE8) { // cate xmlns and style inline
	...
	symbols: {
		...
		rect: function (left, top, width, height, options) {
			...
			// Has radius cate arcs for the corners
			} else {
...
if (useCanVG) {
	...
	/**
	 * Start with an empty symbols object. This is needed when exporting is used (exporting.src.js will cate a few symbols), but
		...
		// Create new label
			...
			this.label =
				this.axis.chart.renderer.text(str, 0, 0, options.useHTML)		// dummy positions, actual position updated with setOffset method in columnseries
					...
					.add(group);							// cate to the labels-group
		}
	},
...
Axis.prototype = {
	...
	init: function (chart, userOptions) {
		...
		// Register
		if (inArray(axis, chart.axes) === -1) { // don't cate it again on Axis.update()
	...
	defaultLabelFormatter: function () {
		...
		if (formatOption) {
			...
			// Decide whether we should cate a numeric symbol like k (thousands) or M (millions).
		...
		if (ret === UNDEFINED) {
			if (value >= 1000) { // cate thousands separators
	...
	getLinearTickPositions: function (tickInterval, min, max) {
		...
		while (pos <= roundedMax) {
			...
			// Always cate the raw tickInterval, not the corrected one.
	...
	setTickPositions: function (secondPass) {
		...
		if (!isLinked) {
			...
			// and max are equal and tickPositions.length is 1. In this case, cate some padding
	...
	render: function () {
		...
		if (hasData || isLinked) {
			...
			if (tickPositions.length) { // #1300
				...
				// we need to cate a tick mark and grid line at the left edge of the X axis.
...
Legend.prototype = {
	...
	renderItem: function (item) {
		...
		if (!li) { // generate it once, later move it
			...
			// Generate the list item text and cate it to the group
...
Chart.prototype = {
	...
	getAxes: function () {
		...
		// make sure the options are arrays and cate some members
...
Point.prototype = {
	 ...
	 * @param {Boolean} accumulate Whether to cate to the previous selection. By default,
	...
	firePointEvent: function (eventType, eventArgs, defaultFunction) {
		...
		// cate default handler if in selection mode
...
Series.prototype = {
	...
	getSegments: function () {
		...
		if (pointsLength) { // no cate required for []
	...
	getGraphPath: function () {
		...
		each(series.segments, function (segment) {
			...
			// cate the segment to the graph, or a single point for tracking
			if (segment.length > 1) {
...
var PiePoint = extendClass(Point, {
	...
	init: function () {
		...
		// cate event listener for select
